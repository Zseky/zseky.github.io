...
.requires(
	...
	'game.levels.opening',
	'game.levels.title',
	'game.levels.levelpage',
	'game.levels.gameplay',
	// Database
	'game.database.color_levels'
)
.defines(function(){
    this.START_OBFUSCATION;
	this.FRAMEBREAKER;
	MyGame = ig.Game.extend({
        name: "MJS-Guess-the-Color", // Name of the game, used as part of the key name in localstorage
		...
		init: function() {
			...
			var bgm = this.getMusicVolume();
			var sfx = this.getSoundVolume();
			this.setMusicVolume(bgm);
			this.setSoundVolume(sfx);
		},
        initData: function(){
		...
		setSoundVolume: function(volume){
			...
			// ig.game.save('sound', volume);
			ig.game.sessionData.sound = volume;
		},
		getSoundVolume: function(){
			var volume = ig.game.sessionData.sound;
		...
		setMusicVolume: function(volume){
			// ig.game.save('music', volume);
			if(volume)
				ig.soundHandler.unmuteBGM(true);
			else
				ig.soundHandler.muteBGM(true);
			ig.game.sessionData.music = volume;
			console.log("Save music : " + ig.game.sessionData.music);
		},
		getMusicVolume: function(){
			var volume = ig.game.sessionData.music;
